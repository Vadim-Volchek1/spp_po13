...
class PeanoCurve:
    ...
    def generate_code(self, n):
        ...
        result = []
        result.extend(d_prev); result.append(1j)
        result.extend(i_d); result.append(1j)
        result.extend(i_conj); result.append(1)
        result.extend(d_conj); result.append(-1j)
        result.extend(self.apply_rotation(d_conj, -1j)); result.append(-1j)
        result.extend(self.apply_rotation(d_prev, -1j)); result.append(1)
        result.extend(d_prev); result.append(1j)
        result.extend(i_d); result.append(1j)
...
class PeanoFractalApp:
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(control_frame, text="Кривая Пеано", 
                                font=('Arial', 16, 'bold'), bg='#f0f0f0')
        title_label.pack(pady=10)
        
        params_frame = tk.LabelFrame(control_frame, text="Параметры", 
                                      bg='#f0f0f0', padx=10, pady=10)
        params_frame.pack(fill=tk.X, padx=10, pady=10)
        
        tk.Label(params_frame, text="Уровень рекурсии (1-4):", bg='#f0f0f0').pack(anchor=tk.W)
        self.iter_scale = tk.Scale(params_frame, from_=1, to=4, 
                                    orient=tk.HORIZONTAL, variable=self.iterations,
                                    command=self.on_param_change)
        self.iter_scale.pack(fill=tk.X)
        
        tk.Label(params_frame, text="Размер элемента:", bg='#f0f0f0').pack(anchor=tk.W, pady=(10,0))
        self.calibre_scale = tk.Scale(params_frame, from_=5, to=50, 
                                       orient=tk.HORIZONTAL, variable=self.calibre,
                                       command=self.on_param_change)
        self.calibre_scale.pack(fill=tk.X)
        
        tk.Label(params_frame, text="Скорость отрисовки:", bg='#f0f0f0').pack(anchor=tk.W, pady=(10,0))
        self.speed_scale = tk.Scale(params_frame, from_=1, to=10, 
                                     orient=tk.HORIZONTAL, variable=self.speed)
        self.speed_scale.pack(fill=tk.X)
        
        self.squares_check = tk.Checkbutton(params_frame, text="Показывать квадраты цепи",
        ...
        self.info_label = tk.Label(info_frame, text="Не отрисовано", 
        ...
        self.draw_btn = tk.Button(btn_frame, text="Начать отрисовку", 
        ...
        self.screenshot_btn = tk.Button(btn_frame, text="Скриншот (PNG)", 
        ...
        if self.show_squares.get():
            ...
            for x, y in self.scaled_points:
                self.canvas.create_rectangle(
                    x - size/2, y - size/2,
                    x + size/2, y + size/2,
                    outline='lightgray', fill='', tags='square'
                )
    ...
    def take_screenshot(self):
        try:
            ...
            messagebox.showinfo("Скриншот", f"Скриншот сохранён:\n{os.path.abspath(filename)}")
...
class MovingBallApp:
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(control_frame, text="Движущийся шар", 
                                font=('Arial', 16, 'bold'), bg='#f0f0f0')
        title_label.pack(pady=10)
        ...
        # Скорость
        tk.Label(params_frame, text="Скорость движения:", bg='#f0f0f0').pack(anchor=tk.W)
        self.speed_scale = tk.Scale(params_frame, from_=0.5, to=15.0, 
                                     orient=tk.HORIZONTAL, variable=self.speed,
                                     resolution=0.5, length=200)
        self.speed_scale.pack(fill=tk.X, pady=5)
        
        # Цвет шара
        tk.Label(params_frame, text="Цвет шара:", bg='#f0f0f0').pack(anchor=tk.W, pady=(10,0))
        self.color_combo = ttk.Combobox(params_frame, textvariable=self.ball_color,
        ...
        # Минимальный размер
        tk.Label(params_frame, text="Минимальный размер (вдали):", bg='#f0f0f0').pack(anchor=tk.W, pady=(10,0))
        self.min_size_scale = tk.Scale(params_frame, from_=10, to=80, 
                                        orient=tk.HORIZONTAL, variable=self.ball_size_min,
                                        command=lambda x: self.update_ball())
        self.min_size_scale.pack(fill=tk.X)
        
        # Максимальный размер
        tk.Label(params_frame, text="Максимальный размер (вблизи):", bg='#f0f0f0').pack(anchor=tk.W, pady=(10,0))
        self.max_size_scale = tk.Scale(params_frame, from_=50, to=200, 
                                        orient=tk.HORIZONTAL, variable=self.ball_size_max,
                                        command=lambda x: self.update_ball())
        ...
        self.info_label = tk.Label(info_frame, text="Ожидание", 
                                    bg='#f0f0f0', justify=tk.LEFT)
        self.info_label.pack()
        ...
        self.reset_btn = tk.Button(btn_frame, text="Сброс позиции", 
        ...
        status_frame = tk.LabelFrame(control_frame, text="Статус", 
                                      bg='#f0f0f0', padx=10, pady=10)
        status_frame.pack(fill=tk.X, padx=10, pady=10)
        
        self.status_label = tk.Label(status_frame, text="● Движение активно", 
    ...
    def get_ball_size(self):
        ...
        size = self.ball_size_min.get() + (self.ball_size_max.get() - self.ball_size_min.get()) * progress
        return int(size)
    ...
    def update_ball(self):
        ...
        self.ball = self.canvas.create_oval(x1, y1, x2, y2, 
                                            fill=self.ball_color.get(), 
                                            outline='black', width=2)
        
        # Блик
        highlight_size = size // 3
        self.highlight = self.canvas.create_oval(
            self.x_pos - highlight_size, center_y - highlight_size,
            self.x_pos, center_y,
            fill='white', outline=''
        )
